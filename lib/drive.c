...
int Stepper_motor_write(const struct stepper_motor *motor, uint16_t channel, uint16_t *channel_range) { 
	...
	switch(pos & 0x03) { 
		case 0: ret+=gpio_pin_set_dt(&(motor->step),0);	//(0b10&(1<<0))?1:0); 
			break; 
		case 1: ret+=gpio_pin_set_dt(&(motor->step),1);	//(0b11&(1<<0))?1:0);  
			break; 
		case 2: ret+=gpio_pin_set_dt(&(motor->step),1);	//(0b01&(1<<0))?1:0);  
			break; 
		case 3: ret+=gpio_pin_set_dt(&(motor->step),0);	//(0b00&(1<<0))?1:0); 
			break;
	}
	return ret;
} 

// returns the number of ticks(delay) required to to achieve input speed (in rpm)
