...
/*
 * We are manually tuning BETA to fix drift.
 * A higher value forces the filter to trust the magnetometer more.
 * The old value (0.05) was too low and trusted the drifting gyro too much.
 */
#ifndef BETA
...
static inline void quat_Normalization(struct quaternion* q) {
    ...
    if (norm == 0.0f) return; // handle divide by zero
    q->q1 /= norm;
    q->q2 /= norm;
    q->q3 /= norm;
    q->q4 /= norm;
