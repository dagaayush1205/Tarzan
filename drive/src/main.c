...
uint32_t linear_actuator_pwm_interpolation(uint16_t linear_actuator_movement,
                                           uint32_t *pwm_range) {
...
int linear_actuator_write(int i, int dir) {
  if (pwm_motor_write(&(motor[i]),
                      linear_actuator_pwm_interpolation(dir, pwm_range))) {
  ...
  while (true) {
    ...
    if (flag == 0) {
      ...
      if (ch[8] > 1000) {
        ...
        cmd.linear_x =
            sbus_velocity_interpolation(ch[1], linear_velocity_range);
        ...
        cmd.angular_z =
            sbus_velocity_interpolation(neutral, angular_velocity_range);
        cmd.linear_x =
            sbus_velocity_interpolation(neutral, linear_velocity_range);
