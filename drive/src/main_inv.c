...
/* imu uart */
static const struct device *const imu_uart = DEVICE_DT_GET(DT_ALIAS(imu_uart));
...
} drive_com = {.msgq_rx = &drive_msgq}, inv_com = {.msgq_rx = &inv_msgq},
...
void telemetry_tx_work_handler(struct k_work *telemetry_tx_work_ptr) {
  struct com_tx_arg *com_info =
      CONTAINER_OF(telemetry_tx_work_ptr, struct com_tx_arg,
                   telemetry_tx_work_item); // changed type from com_arg to
                                            // com_tx_arg for the same error

  if (k_mutex_lock(&arm_imu_mutex, K_NO_WAIT) != 0)
    return;
...
void auto_drive_work_handler(struct k_work *auto_drive_work_ptr) {
  // struct drive_arg *drive_info =
  //     CONTAINER_OF(auto_drive_work_ptr, struct drive_arg,
  //     auto_drive_work_item);
...
void arm_imu_work_handler(struct k_work *imu_work_ptr) {
  ...
  arm_info->dir[0] = update_proportional(
      imu_com.msg_rx.inv.turn_table, (arm_info->baseLink.yaw - arm.rover.yaw));
  arm_info->dir[1] = update_proportional(imu_com.msg_rx.inv.first_link,
                                         -1 * arm_info->lowerIMU.pitch);
  arm_info->dir[2] = update_proportional((imu_com.msg_rx.inv.second_link) -
                                             (imu_com.msg_rx.inv.first_link),
                                         arm_info->upperIMU.pitch);
  arm_info->dir[3] =
      update_proportional((imu_com.msg_rx.inv.pitch), arm.endIMU.pitch);
  arm_info->dir[4] =
      update_proportional(imu_com.msg_rx.inv.roll, arm.endIMU.roll);
  k_mutex_unlock(&arm_imu_mutex);
...
void arm_imu_data_work_handler(struct k_work *imu_data_work_ptr) {
  ...
  if (!process_pitch_roll(&imu_com.msg_rx.imu.secondLink,
                          &(arm_info->upperIMU)))
    error_mssg_flag = error_mssg_flag | 0x2000;
  // printk("No data from imu_upper_joint: %s\n", imu_upper_joint->name);

  if (!process_pitch_roll(&imu_com.msg_rx.imu.differential,
                          &(arm_info->endIMU)))
