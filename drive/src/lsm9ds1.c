...
if (sensor_attr_set(imu_dev, SENSOR_CHAN_GYRO_XYZ,
  ...
  if (calibrationlsm(imu_dev, &imu_data) != 0) {
    ...
    if (calibrate_magnetometer(mag_dev, &imu_data, 20000) != 0) {
        printk("Magnetometer calibration FAILED. Halting.\n");
        return 1;
    }
    
    printk("All calibrations complete.\n");
    ...
    //change the orientation if needed after testing
        ...
        float az = (float)sensor_value_to_double(&accel[2])-9.80665;
        ...
        float mx = (float)sensor_value_to_double(&mag[0])-imu_data.mag_offset[0];
        float my = (float)sensor_value_to_double(&mag[1])-imu_data.mag_offset[1];
        float mz = (float)sensor_value_to_double(&mag[2])-imu_data.mag_offset[2];

       printk("Before madgiwck filter: ax= %.1f, ay=%.1f, az=%.1f \n",ax,ay,az);
       printk("Before madgiwck filter: gx= %.1f, gy=%.1f:, gz=%.1f \n",gx,gy,gz);
       printk("Before madgiwck filter: mx= %.1f, my=%.1f, mz=%.1f \n",mx,my,mz);
       printk("\n");
    //9dof madgwick filter
    imu_filter(ax, ay, az, gx, gy, gz, mx, my, mz, dt);
       printk("AFTER madgiwck filter: ax= %.1f, ay=%.1f, az=%.1f \n",ax,ay,az);
       printk("AFTER madgiwck filter: gx= %.1f, gy=%.1f, gz=%.1f \n",gx,gy,gz);
       printk("AFTER madgiwck filter: mx= %.1f, my=%.1f, mz=%.1f \n",mx,my,mz);
       printk("\n");
