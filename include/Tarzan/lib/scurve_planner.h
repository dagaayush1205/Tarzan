...
static inline void calculate_phase(float v0, float v1, float a_max, float j_max, float* t_j, float* t_a, float* dist_a) {
...
static inline bool scurve_plan_profile(scurve_profile_t *p, const scurve_constraints_t *limits, float dist, float v_start, float v_end)
{
    p->limits= *limits;
    p->dist= dist;
    p->v_start= v_start;
...
static inline float scurve_evaluate_velocity(const scurve_profile_t *p, float t) {
