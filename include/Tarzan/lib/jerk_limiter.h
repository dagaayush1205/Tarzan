...
#define JERK_LIMITER_DT_SEC 0.01f
...
static inline void jerk_limiter_init(struct jerk_limiter_t *lim, float v0,
                                     float a0, float v_max, float a_max,
                                     float j_max) {
  lim->v_last = v0;
  lim->a_last = a0;
  lim->v_max = v_max;
...
static inline float jerk_limiter_step(struct jerk_limiter_t *lim,
                                      float v_target, uint64_t dt) {
  // accumulate dt
  lim->dt_sec += dt;

  if (lim->dt_sec < JERK_LIMITER_DT_SEC) {
    return lim->v_last;
  }

  float dt_sec = lim->dt_sec;
